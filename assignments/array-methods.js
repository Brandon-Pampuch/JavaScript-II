...
totalAmount = runners.reduce(function(total, runner){
  return total + runner.donation
},0)

ticketPriceTotal.push(totalAmount)
...
// Problem 1
// filter out the cheap skates! Create an array of anyone who donated more than 100.

let notCheap = []

notCheap = runners.filter(function(runner){
...
console.log(notCheap)
// Problem 2
//Get an attendance list ready and sort the runners alphabitically so that they can easilyt find their name on the list.
let listOfRunners = [];

listOfRunners = runners.map(function(runner){
    return (`${runner.last_name} ${runner.first_name} `)
})

console.log(listOfRunners.sort())

// Problem 3
//The reace has been cancelled due to a zombie alert.  Collect all the emails to send out an alert.

let emailz = [];

runners.forEach(function(runner){
    emailz.push(runner.email)
})
